...
public class LicenseKey {
	    public String solution(String S, int K) {
	        ...
	        int numOfParts = sb.length() / K;
	        int firstLen = sb.length() % K;
	        if(numOfParts == 0) return total;
	        sb = new StringBuilder();
	        if(firstLen != 0) {
	            sb.append(total.substring(0, firstLen)).append("-");
	        // System.out.println(total.substring(0, firstLen));
	        }
	        while(numOfParts > 0) {
	            sb.append(total.substring(firstLen, firstLen + K));
	            if(numOfParts != 1)sb.append("-");
	            firstLen += K;
	            numOfParts--;
	        }
